    ...
    //     btn.innerText = "AUTHORIZING..."; btn.classList.add('btn-loading');
    //     const email = document.getElementById('login-email').value;
    //     const pass = document.getElementById('login-pass').value;
    //     const res = await this.post('login', { email, password: pass });
    //     if (res.success) {
    ...
    //     } else {
    //       alert("Akses Ditolak!");
    //       btn.innerText = "AUTHORIZE"; btn.classList.remove('btn-loading');
    //     }
    //   },
    ...
    try {
        ...
        // Pola darurat: Jika error (CORS/Network), namun ingin bypass untuk testing
        // Hapus bagian ini jika sudah masuk tahap produksi (Production)
        /*
        localStorage.setItem('sk_token', 'test-token');
        localStorage.setItem('sk_role', 'admin');
        localStorage.setItem('sk_email', email);
        location.reload();
        */
      ...
      renderTable(rows) {
        ...
        const viewMode = document.getElementById('view-mode').value; // Ambil mode active/trash
        ...
        head.innerHTML = `<tr>${fields.map(f => `<th class="p-6 text-left">${this.schema[f]?.label || f}</th>`).join('')}<th class="p-6 text-right">Actions</th></tr>`;
        ...
        body.innerHTML = rows.map(row => `
          ...
          if (s?.type === 'currency' && !isNaN(val) && val !== '-') {
            val = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(val);
          }
          return `<td class="p-6 font-medium text-slate-600">${val}</td>`;
        }).join('')}
            <td class="p-6 text-right space-x-2">
                ${this.modes.edit.can ? `<button onclick='app.openForm(${JSON.stringify(row).replace(/'/g, "&apos;")})' class="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase hover:bg-blue-600 hover:text-white transition-all"><i class="fa-solid fa-pen"></i> Edit</button>` : ''}
                ${viewMode === 'active' ? `<button onclick="app.remove('${row.id}')" class="px-3 py-1.5 bg-red-50 text-red-500 rounded-lg text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition-all"><i class="fa-solid fa-trash"></i> Delete</button>` : ''}
            </td>
          </tr>`).join('');
      },
  ...
  `;
      ...
      async studioMigrate() {
        ...
        fieldNodes.forEach(n => {
          ...
          fields.push({
            ...
            disabled: n.querySelector('.st-disabled').checked,
            formula: n.querySelector('.st-formula').value || null,
            // Logic Relasi Standar (LOOKUP)
            relTable: n.querySelector('.st-rel-table')?.value || '',
            relField: n.querySelector('.st-rel-field')?.value || '',

            // --- INFO 3 SAKTI UNTUK AUTOFILL ---
            // Mengambil info dari dropdown Autofill yang kita buat tadi
        ...
        btn.innerText = "MIGRATING...";
        // Kirim data ke BE. Pastikan BE Juragan sudah siap menerima key autoTrigger, autoTable, autoCol
        const res = await this.post('migrate', { tableName, fields });
        if (res.success) {
          alert("Tabel Berhasil Lahir!");
          location.reload();
        } else {
   ...
   // =============================
      ...
      filterTable(query) {
        ...
        // Filter cerdas: cari di semua kolom
...
`;
      ...
      async loadResource(forceRefresh = false) {
        ...
        // --- PERBAIKAN: CACHE CLEARING ---
        // Jika juragan memaksa refresh, kita kosongkan cache agar tidak muncul data lama
        if (forceRefresh) {
        ...
        const d = await this.get({
          ...
          table: this.currentTable,
          viewMode: vm,
          _t: forceRefresh ? Date.now() : null // Cache buster untuk URL
        });
        ...
        if (!d || d.success === false) {
          // Jika data dari BE kosong total, kita harus bersihkan UI
          if (d.message === "No data found") {
            this.resourceCache[this.currentTable] = [];
            this.renderTable([]);
            return;
          }
        ...
        if (d.success) {
          // Pastikan d.rows tidak null/undefined (jika kosong harus array [])
          const rows = d.rows || [];

          this.schema = d.schema;
          this.modes = d.modes;

          // Update Cache (Data terbaru dari Sheets menimpa data lama)
          ...
          // UI Logic: Hak akses Create New
          if (btnAdd) {
            this.modes.add?.can ? btnAdd.classList.remove('hidden') : btnAdd.classList.add('hidden');
          }

          // Render data ke tabel (Angka di dashboard ikut terupdate karena cache sudah segar)
          this.renderTable(rows);

          // Jika dashboard sedang terbuka, otomatis update angkanya
